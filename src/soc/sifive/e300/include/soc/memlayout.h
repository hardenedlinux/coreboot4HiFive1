...
/* Careful: 'INCLUDE <filename>' must always be at the end of the output line */
	...
	#define BOOTBLOCK(addr, sz) \
		...
		_ = ASSERT(SIZEOF(.text) + SIZEOF(.data) <= sz, STR(bootblock exceeded its allotted size! (sz))); \
	...
	#define ROMSTAGE(addr, sz) \
		...
		_ = ASSERT(SIZEOF(.text) + SIZEOF(.data) <= sz, STR(romstage exceeded its allotted size! (sz))); \
	...
	#define RAMSTAGE(addr, sz) \
		...
		_ = ASSERT(SIZEOF(.text) + SIZEOF(.data) <= sz, STR(ramstage exceeded its allotted size! (sz))); \
